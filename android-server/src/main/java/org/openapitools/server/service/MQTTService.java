...
public class MQTTService extends Service {
    ...
    private void configureMQTT(){
        ...
        mqttAndroidClient.setCallback(new MqttCallbackExtended() {
            ...
            public void connectComplete(boolean b, String s) {
                if (!subscribed) {
                    subscribeTopic(getApplicationContext(), "EmergencyAlerts");
    ...
    private void executeAPI(JSONObject data) throws JSONException {
			...
			case "Emergencies":
                try {
                    EmergenciesResponse emergenciesresponse = gson.fromJson(String.valueOf(data), EmergenciesResponse.class);
                    new EmergenciesResource(getApplicationContext()).executeMethod(emergenciesresponse);

                    //TODO Choose what type of notification to show (toast or notification in the bar)
                    showNotification("Resource Execution: "+ emergenciesresponse.getResource()," Method: "+ emergenciesresponse.getMethod());
                    showToastInIntentService("Resource Execution: "+ emergenciesresponse.getResource() + " | Method: "+ emergenciesresponse.getMethod());

                    //Intent, Broadcast, Insert Notification to send and show info to MainActivity to show in ListView

                    Intent intentEmergencies = new Intent();
                    intentEmergencies.setAction("NOW");

                    LocalBroadcastManager.getInstance(getApplicationContext()).sendBroadcast(intentEmergencies);
                    new AsyncInsertNotification().execute(new NotificationClass(emergenciesresponse.getResource(),emergenciesresponse.getMethod()));
                    //////

                } catch (Exception e) {
                    Log.e("Err EmergenciesResponse", e.getMessage());
                }
				break;
			...
			case "Location":
                try {
                    LocationResponse locationresponse = gson.fromJson(String.valueOf(data), LocationResponse.class);
                    new LocationResource(getApplicationContext()).executeMethod(locationresponse);

                    //TODO Choose what type of notification to show (toast or notification in the bar)
                    showNotification("Resource Execution: "+ locationresponse.getResource()," Method: "+ locationresponse.getMethod());
                    showToastInIntentService("Resource Execution: "+ locationresponse.getResource() + " | Method: "+ locationresponse.getMethod());

                    //Intent, Broadcast, Insert Notification to send and show info to MainActivity to show in ListView

                    Intent intentLocation = new Intent();
                    intentLocation.setAction("NOW");

                    LocalBroadcastManager.getInstance(getApplicationContext()).sendBroadcast(intentLocation);
                    new AsyncInsertNotification().execute(new NotificationClass(locationresponse.getResource(),locationresponse.getMethod()));
                    //////

                } catch (Exception e) {
                    Log.e("Err LocationResponse", e.getMessage());
                }
				break;
